...
class ZSKMixin(object):
    def zsk_expired(self, rname, rrr, keyset):
        ...
        if not isinstance(krec, KeySet):
            self.rolllog_log(
                LOG.ERR, rname, '"%s"\'s keyrec is not a set keyrec' %
                keyset)
            return False
        if not krec.keys:
            self.rolllog_log(
        ...
        if waitsecs >= 0:
            self.rolllog_log(
                LOG.EXPIRE, rname, '        ZSK expiration in %s' %
                datetime.timedelta(seconds=waitsecs))
        else:
            waitsecs = cronus - rolltime
            self.rolllog_log(
                LOG.EXPIRE, rname, '        ZSK expired %s ago' %
                datetime.timedelta(seconds=waitsecs))
    ...
    def zsk_phase2(self, rname, rrr, *skipargs):
        ...
        if not rrr.get('keyrec'):
            self.rolllog_log(
                LOG.ERR, rname, 'ZSK phase 2:  no keyrec for zone specified')
            return 2
        if not krf:
            self.rolllog_log(
                LOG.ERR, rname,
        ...
        if not ret:
            self.rolllog_log(
                LOG.ERR, rname, 'ZSK phase 2:  unable to reload zone')
    ...
    def zsk_phase4(self, rname, rrr, *skipargs):
        ...
        if not rrr.get('keyrec'):
            self.rolllog_log(
                LOG.ERR, rname, 'ZSK phase 4:  no keyrec for zone specified')
            return -1
        if not krf:
            self.rolllog_log(
                LOG.ERR, rname,
        ...
        if not self.signer(rname, 'ZSK phase 4a', krf):
            self.rolllog_log(
                LOG.ERR, rname, 'ZSK phase 4:  unable to adjust ZSK keyrec')
            return -1
        ...
        if not self.loadzone(rname, rrr, 'ZSK phase 4'):
            self.rolllog_log(
                LOG.ERR, rname, 'ZSK phase 4:  unable to reload zone')
