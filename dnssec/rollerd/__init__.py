...
class RollerD(
    ...
    krollmethod = defs.RM_ENDROLL  # Rollover calculation to use for KSKs.
    zrollmethod = defs.RM_ENDROLL  # Rollover calculation to use for ZSKs.
    ...
    def main(self, args):
        ...
        if (self.opts[defs.OPT_DTCONF] and
                os.path.exists(self.opts[defs.OPT_DTCONF])):
            self.dtconfig = self.opts[defs.OPT_DTCONF]
        ...
        if not self.rollmgr_dropid():
            ...
            self.rolllog_log(
                LOG.ALWAYS, '', 'another pyrollerd tried to start')
            self.cleanup()
        ...
        if self.eventmaster == defs.EVT_FULLLIST:
            self.rolllog_log(LOG.ALWAYS, '', ' ')
            self.full_list_event_loop()
        elif self.eventmaster == defs.EVT_QUEUE_SOON:
            self.rolllog_log(LOG.ALWAYS, '', ' ')
    ...
    def eminent_domains(self):
        ...
        if not self.rrfokay(''):
            self.rolllog_log(
                LOG.FATAL, '',
                'rollrec file "%s" invalid' % self.rollrecfile)
            return False
    ...
    def full_list_event_loop(self):
        ...
        while 42:
            ...
            if self.singlerun:
                self.rolllog_log(
                    LOG.INFO, '',
                    'rollover manager shutting down at end of single-run '
                    'execution')
                self.halt_handler()
                sys.exit(0)
    ...
    def rollkeys(self):
        ...
        for rname in self.rollrec_names():
            ...
            if self.queued_int:
                self.rolllog_log(
                    LOG.INFO, rname,
                    'received immediate shutdown command')
                self.halt_handler()
            ...
            if not rrr.is_active:
                self.rolllog_log(LOG.TMI, rname, 'is a skip rollrec')
                continue
            ...
            if 'kskphase' not in rrr:
                self.rolllog_log(LOG.TMI, rname, 'new kskphase entry')
                self.nextphase(rname, rrr, 0, 'ksk')
            if 'zskphase' not in rrr:
                self.rolllog_log(LOG.TMI, rname, 'new zskphase entry')
            ...
            if self.ksk_expired(rname, rrr, 'kskcur'):
                if rrr.zskphase == 0:
                    self.rolllog_log(
                        LOG.TMI, rname, 'current KSK has expired')
                self.ksk_phaser(rname, rrr)
            else:
                self.rolllog_log(
                    LOG.TMI, rname, 'current KSK still valid')

                # If this zone's current ZSK has expired, we'll get it rolling.
                if self.zsk_expired(rname, rrr, 'zskcur'):
                    if rrr.zskphase == 0:
                        self.rolllog_log(
                            LOG.INFO, rname, 'current ZSK has expired')
                    self.zsk_phaser(rname, rrr)
                else:
                    self.rolllog_log(
                        LOG.TMI, rname, 'current ZSK still valid')
            ...
            if self.alwayssign and not self.wassigned:
                ...
                if ret != 0:
                    self.rolllog_log(
                        LOG.ERR, 'signing %s failed!' % rname)
    ...
    def rrfokay(self, mp=''):
        ...
        if ret is False:
            if self.rrferrors == 0:
                # self.rolllog_log(LOG.ERR, mp, err)
                print(err, file=sys.stderr)
                self.rrferrors = defs.MAXRRFERRS
            else:
                self.rrferrors -= 1
            return False
    ...
    def optsandargs(self):
        ...
        self.singlerun = self.opts[defs.OPT_SINGLERUN]
        self.foreground = self.opts[defs.OPT_FOREGROUND]
        self.alwayssign = self.opts[defs.OPT_ALWAYSSIGN] or False
        self.pidfile = self.opts[defs.OPT_PIDFILE]
        self.lockfile = self.opts['lockfile']
        self.sockfile = self.opts['sockfile']
        self.realm = self.opts[defs.OPT_REALM]
        self.verbose = self.opts[defs.OPT_VERBOSE]
        self.logfile = (
            self.opts[defs.OPT_LOGFILE] or
            self.dtconf.get(defs.DT_LOGFILE))
        self.loglevel = (
            self.opts[defs.OPT_LOGLEVEL] or
            self.dtconf.get(defs.DT_LOGLEVEL) or defs.LOG_DEFAULT)
        self.logtz = (
            self.opts[defs.OPT_LOGTZ] or
            self.dtconf.get(defs.DT_LOGTZ))
        self.sleeptime = (
            self.opts[defs.OPT_SLEEP] or
            int(self.dtconf.get(defs.DT_SLEEP)) or defs.DEFAULT_NAP)
        self.dtcf = self.opts[defs.OPT_DTCONF] or self.dtconfig
        self.display = self.opts[defs.OPT_DISPLAY] or False
        self.gzsargs = self.opts[defs.OPT_ZSARGS] or ''
        self.username = (
            self.opts[defs.OPT_USERNAME] or
            self.dtconf.get(defs.DT_USERNAME) or '')
        self.xqtdir = self.opts[defs.OPT_DIR] or '.'
        self.zonesigner = (
            self.opts[defs.OPT_ZONESIGNER] or
            self.dtconf.get(defs.OPT_ZONESIGNER) or '/usr/sbin/zonesigner')

        # Check for autosign presence or absence.
        self.autosign = (
            self.opts[defs.OPT_AUTOSIGN] or
            (self.dtconf.get(defs.DT_AUTOSIGN) == '1') or False)

        # Determine whether or not we'll load zones.
        self.zoneload = (
            not self.opts[defs.OPT_NORELOAD] or
            (self.dtconf.get(defs.DT_LOADZONE) == '1'))

        # Show the version number if requested
        if self.opts[defs.OPT_HELP]:
            self.usage()
        if self.opts[defs.OPT_VERSION]:
            self.version()

        # Check for a rollrec file name.
        self.rollrecfile = (
            self.opts[defs.OPT_RRFILE] or
            '/etc/dnssec-tools/dnssec-tools.rollrec')
        ...
        if not self.xqtdir:
            print(
                'pyrollerd:  no execution directory defined', file=sys.stderr)
            self.opterrs += 1
        ...
        self.rndcopts = self.dtconf.get(defs.DT_RNDCOPTS)
    ...
    def cleanup(self):
        ...
        '''
        if self.loglevel == LOG.TMI:
            self.rolllog_log(LOG.ALWAYS, '', 'cleaning up...')
        sys.exit(0)
    ...
    def signer(self, rname, zsflag, krr=None):
        ...
        # self.rolllog_log(LOG.INFO, rname, 'executing "%s"' % cmdstr)
    ...
    def runner(self, rname, cmd, krf, negerrflag):
        ...
        self.rolllog_log(LOG.TMI, rname, 'executing "%s"' % cmd)
        ...
        if rcode != 0:
            self.rolllog_log(
                LOG.ERR, rname, 'execution error for command "%s"' % cmd)
            self.rolllog_log(LOG.ERR, rname, 'error return - %d' % rcode)
            self.rolllog_log(LOG.ERR, rname, 'error output - "%s"' % out)
    ...
    def rrfchk(self):
        ...
        for rname in self.rollrec_names():
            ...
            rrr = self.rollrec_fullrec(rname)
            if not rrr:
                self.rolllog_log(LOG.ERR, rname, 'no rollrec defined for zone')
                continue
    ...
    def nextphase(self, rname, rrr, phase, phasetype):
        ...
        if phase == 1:
            self.rolllog_log(
                LOG.TMI, rname, '>>> starting %s rollover' % phasetype.upper())
        else:
            self.rolllog_log(
                LOG.TMI, rname, '>>> moving to %s phase %d' %
                (phasetype.upper(), phase))

        # This is the source of the log messages that look like
        # "KSK phase 3" and "ZSK phase 4".
        self.rolllog_log(
            LOG.PHASE, rname, '%s phase %d' % (phasetype.upper(), phase))
    ...
    def zonemodified(self, rrr, rname):
        ...
        if zfutime > zfstime:
            self.rolllog_log(LOG.PHASE, rname, 'zonefile modified; re-signing')

            # Sign -- just sign -- the zone.
            if self.signer(rname, rrr.phaseargs, krf):
                self.rolllog_log(LOG.TMI, rname, 'rollerd signed zone')
            else:
                self.rolllog_log(LOG.ERR, rname, 'unable to sign zone')
    ...
    def loadzone(self, rname, rrr, phase):
        ...
        if not self.zoneload:
            self.rolllog_log(
                LOG.INFO, rname, 'not reloading zone for %s' % phase)
            return False

        # Reload the zone for real.
        self.rolllog_log(LOG.INFO, rname, 'reloading zone for %s' % phase)
